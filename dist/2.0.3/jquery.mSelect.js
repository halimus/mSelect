...
(function($){
    ...
    $.fn.mSelect = function(options) {
        ...
        plugin.initialize = function(){
            ...
            if(settings.topNav == true){
                domPositioning = '<"top"lBpf>rt<"bottom"ip><"clear">';
            }
         ...
         * 
         */
        this.setSelectedIds = function(selectedIds, selectedLabels) {
            selectedIds = convert_ids_toString(selectedIds);
            if(selectedIds.length > 0){
                settings.selectedIds = selectedIds; 
                if(selectedLabels != undefined && selectedLabels != null) {
                    if(settings.returnSelectedLabels.enable === true){
                        var arr = [];
                        $.each(selectedIds , function(key, value) {
                            if(selectedLabels[key] != undefined){
                                arr.push({id: value, label: selectedLabels[key]});
                            }
                        });
                        settings.fullSelectedLabels = arr;
                        getUniqueLabels();
                    }
                }
                reload_mSelect();
